  ...
  public decimal CalculateServiceCharge(int bookingId)
      ...
      filter: bs => bs.BookingId == bookingId
        ...
        public Invoice CreateInvoice(int bookingId, decimal discount, string paymentMethod, int? staffId, string? note = null)
        {
            ...
            // Ki?m tra ð? có hóa ðõn chýa
   var existingInvoice = _unitOfWork.InvoiceRepository.GetAll(
    filter: i => i.BookingId == bookingId
    ).FirstOrDefault();

     if (existingInvoice != null) throw new Exception("Booking này ð? có hóa ðõn!");
...
BookingId = bookingId,
 ...
 _unitOfWork.Save();
   ...
   return invoice;
    ...
    if (invoice == null) throw new Exception("Không t?m th?y hóa ðõn!");
     ...
     filter: bs => bs.BookingId == invoice.BookingId,
          ...
          {
                var sheet = package.Workbook.Worksheets.Add("Hóa ðõn");

         // Thông tin chung
          sheet.Cells[1, 1].Value = "HÓA ÐÕN THANH TOÁN";
                sheet.Cells[2, 1].Value = $"M? hóa ðõn: {invoice.InvoiceId}";
 ...
 sheet.Cells[row, 2].Value = invoice.ServiceCharge;
  ...
  row++;
       sheet.Cells[row, 1].Value = $"Phýõng th?c: {invoice.PaymentMethod}";
 ...
 public void ExportAllInvoicesToExcel(string filePath)
 {
   ...
   using (var package = new ExcelPackage())
      {
     var sheet = package.Workbook.Worksheets.Add("Danh sách Hóa ðõn");

          string[] headers = { "M? HÐ", "Khách hàng", "Ph?ng", "Ti?n ph?ng", "Ti?n DV", "Gi?m giá", "T?ng c?ng", "Thanh toán", "Ngày" };
